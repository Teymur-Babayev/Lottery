...
var current_lottery_game_date = '', loginUserId = 0, totalTicCount = '?', selected_ticketType = 0;
const TicketSearch = () => {
    ...
    if (sessionStorage.getItem('authUserToken')) {
      ...
      _sendController([constant.GET_TICKETS, user.user_id], (response) => {
        if (response !== constant.FORBIDDEN_USER) {
          ...
          setTickets(response[1]);  // Ensure the response is an array, defaulting to an empty array
  ...
  const show_selected_option_value = (e) => {
    if (e === constant.FIRST_KEY) {
      ...
      _sendController([constant.GET_NEW_TICKETS, current_lottery_game_date], (response) => {
        if (response !== constant.FORBIDDEN_USER) {
          setAllowTic(response[0][0].allowTic); setMyTic(response[0][0].myTic); setTickets(response[1]); setTotalTickets(response[1].length); setCurrentPage(1);
  ...
  const setSearchValue = (keyLocation, keyType) => {
    ...
    // console.log(firstSearchKey,secondSearchKey, thirdSearchKey,forthSearchKey,fifthSearchKey, 135)
    ...
    const filterTickets = (pattern, tickets) => {
        ...
        return tickets.filter(ticket => {
          ...
          if (!ticketNum) return false;  // Handle case where ticketNum might be missing
          ...
          for (let i = 0; i < pattern.length; i++) {
            ...
            // If the pattern character is '*', it can match any digit
            if (patternChar === '*') {
              continue;
            }
    
            // If the pattern character is a number, it must match the corresponding ticket character
  ...
  // Filter tickets based on the search pattern
                ...
                <div className="mt-[8rem] flex flex-wrap gap-7">
                  {
                    currentTickets.map((data, index) => {
                      return (
                        <Ticket key={index} ticketNum={data.ticket} wallet_address={data.wallet_address} />
                      )
                    })
                  }
                </div>
