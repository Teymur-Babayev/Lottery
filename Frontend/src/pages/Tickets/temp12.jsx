...
const TicketSearch = () => {
  ...
  const [sencondSearchKey, setSencondSearchKey] = useState('*');
    ...
    if (sessionStorage.getItem('authUserToken')) {
      ...
      _sendController([constant.GET_TICKETS, user.user_id], (response) => {
        if (response !== constant.FORBIDDEN_USER) {
          const current_lottery_game_date = response[2];
          const loginUserId = user.user_id;
          const totalTicCount = response[1].length;
          setCurrentPage(1);
          ...
          setTotalTickets(response[1]?.length || 0);
        } else {
  ...
  const setSearchValue = (keyLocation, keyType) => {
    ...
    switch (keyLocation) {
      ...
      case 3:
        ...
        break;
      case 4:
        updateKey(fourthSearchKey, setFourthSearchKey);
        break;
      case 5:
    ...
    // Function to filter tickets based on the current search pattern
  ...
  // Combine the search keys into one pattern string
  const getSearchPattern = () => {
    return `${firstSearchKey}${secondSearchKey}${thirdSearchKey}${fourthSearchKey}${fifthSearchKey}`;
  };

  // Filter tickets based on the search pattern
  const searchPattern = getSearchPattern();
  setTickets(filterTickets(searchPattern, tickets));
                    ...
                    >
                      <div className="absolute flex h-full -rotate-45 transform flex-col items-center justify-between">
                        <button
                          ...
                          // onClick={() =>
                          //   setFirstSearchKey((prev) => String(Number(prev) + 1))
                          // }
                        ></button>
                        <input
                          type="text"
                    ...
                    >
                      <div className="absolute flex h-full -rotate-45 transform flex-col items-center justify-between">
                        ...
                        <input
                          type="text"
                          className="text-gray-900 w-10 rounded border border-[#232626] bg-[#292d2e] text-center text-lg font-bold focus:outline-none"
                          value={sencondSearchKey}
                          onChange={(e) => setSencondSearchKey(e.target.value)}
                        />
                        <button
                          className="triangle-down hover:bg-gray-800 h-3.5 w-6 bg-[#323738]"
                          style={{
