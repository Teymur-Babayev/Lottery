...
export default function SignUp({ isOpen, onClose, handleSignIn }) {
  ...
  const [isAgreementChecked, setIsAgreementChecked] = useState(0);
  const [isGmailChecked, setIsGmailChecked] = useState(0);
  const registerUser = () => {
    console.log(userEmail, userWalletAddress, userPassword, isAgreementChecked, isGmailChecked, 14)
    if (userEmail !== '' && userWalletAddress !== '' && userPassword !== '' && isGmailChecked === 1 && isAgreementChecked === 1) {
      _sendController([constant.REGISTER_USER, userEmail, userWalletAddress, userPassword], (response) => {
        console.log(response, 16)
        if (response === constant.SUCCESS_RESULT) {
          alert(constant.SUCCESS_OPERATOR_MSG);
          setUseEmail('');
          setUserPassword('');
          setIsGmailChecked(0);
          setUserWalletAddress('');
          setIsAgreementChecked(0);
        } else {
          alert(constant.ALEADY_EXIST_MSG);
        }
      });
    } else {
  ...
  const checkUserAgreement = (state) => {
    if (state === 0) {
      isAgreementChecked === 0 ? setIsAgreementChecked(1) : setIsAgreementChecked(0);
    } else {
      isGmailChecked === 0 ? setIsGmailChecked(1) : setIsGmailChecked(0);
    }
  }
              ...
              <div className="flex h-full w-full bg-[#232626]">
                <div className="relative h-full w-full flex-none px-6 py-5">
                  ...
                  <form >
                    ...
                    <div
                      className="text0-white mt-3 flex h-[2.5rem] items-center bg-[#1e2121] pl-[0.5rem] pr-[0.1875rem]"
                      style={{
                        borderRadius: "0.5rem",
                        border: "1px solid #3a4142",
                        gap: "0.5rem",
                    ...
                    <div className="relative mt-5 flex">
                      <label className="custom-checkbox">
                        <input type="checkbox" onChange={() => checkUserAgreement(0)} />
                    ...
                    <div className="relative mt-3 flex">
                      <label className="custom-checkbox">
                        <input type="checkbox" onChange={() => checkUserAgreement(1)} />
                        <span className="checkmark"></span>
                      </label>
                      <span className="mt-1 text-[#b3bec1] font-[700]">
